...
const (
	wasmModuleEnv         = "env"
	wasmErrOutputTooLarge = -1
	wasmErrInvalidOutput  = -2
)
...
func newWasmModule(baseDir string, cfg compiledWasm) (*wasmModule, error) {
	...
	memLimit := cfg.maxMemoryBytes
	if cfg.maxOutputBytes > 0 && memLimit < cfg.maxOutputBytes+65536 {
		memLimit = cfg.maxOutputBytes + 65536
	}
	if memLimit > 0 {
		pages := uint32((memLimit + 65535) / 65536)
		if pages == 0 {
			pages = 1
		}
...
func (m *wasmModule) transform(ctx context.Context, in assetInput) ([]byte, error) {
	host := &wasmHost{
		resPath: []byte(in.ResPath),
		cdnPath: []byte(in.CDNPath),
	}
	hostCtx := withWasmHost(ctx, host)

	instanceName := fmt.Sprintf("%s-%p", filepath.Base(m.path), host)
	modCtx := wazero.NewModuleConfig().
		WithName(instanceName)

	module, err := m.runtime.InstantiateModule(hostCtx, m.compiled, modCtx)
	if err != nil {
	...
	runCtx, cancel := context.WithTimeout(hostCtx, wasmExecutionTimeout(m.cfg.fuel))
	defer cancel()
...
func newWasmRunner(baseDir string, cfg compiledWasm) (*wasmRunner, error) {
...
func (r *wasmRunner) transform(ctx context.Context, in assetInput) ([]byte, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
